...
// ─── MOT DE PASSE GÉRANT ────────────────────────────────────────────────────
// Sur Render : définir la variable d'environnement GERANT_SECRET
// En local   : remplacer "changez-moi-svp" par votre mot de passe
var GERANT_SECRET = process.env.GERANT_SECRET || 'changez-moi-svp';

function authGerant(req, res, next) {
    var token = req.headers['x-gerant-token'] || (req.body && req.body._gerantToken);
    if (token === GERANT_SECRET) return next();
...
// ─── LOGIN GÉRANT ─────────────────────────────────────────────────────────
app.post('/api/gerant/login', function (req, res) {
    var pwd = req.body && req.body.password;
    if (pwd === GERANT_SECRET) return res.json({ success: true });
    return res.status(401).json({ success: false, error: 'Mot de passe incorrect' });
});

app.get('/api/gerant/data', authGerant, function (req, res) {
    var db = read();
    var now = Date.now();
...
app.post('/api/gerant/action', authGerant, function (req, res) {
    var db = read();
    var u = db.users.find(function (x) { return x.user === req.body.user; });
    if (u) {
...
app.post('/api/gerant/chat', authGerant, function (req, res) {
    var db = read();
...
app.post('/api/root/reset', authGerant, (req, res) => {
    var db = read();
...
app.post('/api/root/delete-users', authGerant, (req, res) => {
    var db = read();
